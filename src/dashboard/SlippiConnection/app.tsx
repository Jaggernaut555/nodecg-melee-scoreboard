...
function App() {
    ...
    const [slippiMethod, setSlippiMethod] = useReplicant<SlippiMethod>("slippiMethod", "fileWatcher");
    const [displayedSlippiMethod, setDisplayedSlippiMethod] = React.useState<SlippiMethod>(slippiMethod);
    ...
    return (
        <div>
            ...
            {displayedSlippiMethod == 'realtime' &&
                <div>
                    <label>IP:Port</label>
                    <input disabled={true} onKeyDown={e => handlePressEnter(e)} type='text' placeholder="127.0.0.1:51441"></input>
            ...
            {displayedSlippiMethod == 'fileWatcher' &&
                <div>
                    ...
                    <input type='text' disabled={connectionStatus == 'connected'} onKeyDown={handlePressEnter} value={displayedReplayFolder} onChange={e => setDisplayedReplayFolder(e.target.value)}></input>
                    </div>
                    <div>
            ...
            <div>
                <button disabled={connectionStatus == 'connected'} onClick={() => saveChanges()}>Save</button>
                <button hidden={connectionStatus != 'disconnected'} onClick={() => connectToSlippi()}>Connect</button>
                <button hidden={connectionStatus != 'connected'} onClick={() => disconnectFromSlippi()}>Disconnect</button>
            </div>
            <div>
